...
def send_request_async(request_id: str, query: str, send_time: float) -> None:
    ...
    try:
        ...
        with results_lock:
            results[request_id] = {"error": "Timeout", "send_time": send_time, "success": False}
        ...
        print(f"\n[{datetime.now().strftime('%H:%M:%S')}] Error for {request_id}: {e!s}")
        with results_lock:
            results[request_id] = {"error": str(e), "send_time": send_time, "success": False}
...
def main() -> None:
    ...
    for i in range(6):
        ...
        thread = threading.Thread(target=send_request_async, args=(request_id, query, time.time()))
        thread.start()
        ...
        if results:
            ...
            with requests_lock:
                for i, req_info in enumerate(requests_sent, 1):
                    req_id: str = req_info["request_id"]
                    if req_id in results:
                        res_info: dict[str, Any] = results[req_id]
                        print(f"\n{i}. Request ID: {req_id}")
